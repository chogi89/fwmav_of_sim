...
// ------------------------ //
/*
void msgCallback(const gazebo_msgs::ModelStates& Msg){
    ...
    pose_p_x_c = Msg.pose[1].position.x;
    pose_p_y_c = Msg.pose[1].position.y;
    pose_p_z_c = Msg.pose[1].position.z;
    pose_o_qx_c = Msg.pose[1].orientation.x;
    pose_o_qy_c = Msg.pose[1].orientation.y;
    pose_o_qz_c = Msg.pose[1].orientation.z;
    pose_o_qw_c = Msg.pose[1].orientation.w;
...
int main (int argc, char **argv){
	...
	//ROS_INFO("okokokokokokokokok");
        if(pose_client.call(srv_pose)){
            ...
            //ROS_ERROR("Failed to call service pose_client.");
        }
        ...
        if(count>=5){
            ...
            d_vel_lx = D_SET;
            d_vel_ly = 0;
            d_vel_lz = of_ud_ctrl_input + alt_ctrl_sat_input;
